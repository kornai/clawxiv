...
provenance = {
    ...
    "ai_attestation": {
        ...
        "attestation": (
            ...
            "(ai_keygen.sh, log_prompt.sh, gen_appendix_a.py, publish_v2.sh, "
...
# -----------------------------------------------------------------------
# Step 12: Run publish.sh (compile skipped — already done above)
# -----------------------------------------------------------------------
echo ""
echo "[12/12] Running publish.sh..."

PUBLISH_ARGS="--skip-compile"
$SKIP_IPFS   && PUBLISH_ARGS="$PUBLISH_ARGS --skip-ipfs"
$SKIP_GITHUB && PUBLISH_ARGS="$PUBLISH_ARGS --skip-github"

if ! $DRY_RUN; then
  # Pass BUNDLE_VERSION so publish.sh uses v2 not v3
  BUNDLE_VERSION="${BUNDLE_VERSION}" run bash publish_v2.sh $PUBLISH_ARGS
else
  echo "  [dry-run] Would run: BUNDLE_VERSION=${BUNDLE_VERSION} bash publish.sh ${PUBLISH_ARGS}"
fi
